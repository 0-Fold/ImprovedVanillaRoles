...
using IVR.AddOns.Common;
using IVR.AddOns.Crewmate;
using IVR.AddOns.GhostRoles;
using IVR.AddOns.Impostor;
using IVR.Crewmate;
using IVR.Impostor;
using IVR.Modules;
using IVR.Neutral;
...
public static class Utils
{
    ...
    {
        SaveFile("./IVR_DATA/AlwaysCombos.json");
        SaveFile("./IVR_DATA/NeverCombos.json");
        return;
    ...
    {
        LoadFile("./IVR_DATA/AlwaysCombos.json");
        LoadFile("./IVR_DATA/NeverCombos.json");
        return;
    ...
    public static void ApplySuffix(PlayerControl player)
    {
        ...
        else
        {
            ...
            if (player.AmOwner)
            {
                ...
                {
                    SuffixModes.IVR => name + $"\r\n<color={Main.ModColor}>IVR v{Main.PluginDisplayVersion}</color>",
    ...
    public static void AddAbilityCD(CustomRoles role, byte playerId, bool includeDuration = true)
    {
        ...
        int CD = role switch
        {
            ...
            CustomRoles.Sentry => IVR.Impostor.Sentry.ShowInfoCooldown.GetInt(),
    ...
    public static void AfterPlayerDeathTasks(PlayerControl target, bool onMeeting = false)
    {
        try
        {
            ...
            IVR.Impostor.Sentry.OnAnyoneMurder(target);
    ...
    {
        string f = $"{Environment.GetFolderPath(Environment.SpecialFolder.DesktopDirectory)}/IVR_Logs/";
        string t = DateTime.Now.ToString("yyyy-MM-dd_HH.mm.ss");
        string filename = $"{f}IVR-v{Main.PluginVersion}-{t}.log";
        if (!Directory.Exists(f)) Directory.CreateDirectory(f);
        FileInfo file = new($"{Environment.CurrentDirectory}/BepInEx/LogOutput.log");
        file.CopyTo(filename);
        if (!open) return;
        if (PlayerControl.LocalPlayer != null)
            HudManager.Instance?.Chat?.AddChat(PlayerControl.LocalPlayer, string.Format(GetString("Message.DumpfileSaved"), $"IVR v{Main.PluginVersion} {t}.log"));
