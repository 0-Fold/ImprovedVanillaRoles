...
{
    private static readonly string TEMPLATE_FILE_PATH = "./IVR-DATA/template.txt";
    ...
    public static void CreateIfNotExists()
    {
        ...
        try
        {
            ...
            if (!Directory.Exists(@"IVR-DATA")) Directory.CreateDirectory(@"IVR-DATA");
            var defaultTemplateMsg = GetResourcesTxt($"IVR.Resources.Config.template.{fileName}.txt");
            if (!File.Exists(@"./IVR-DATA/Default_Teamplate.txt")) //default template
            {
                Logger.Warn("Creating Default_Template.txt", "TemplateManager");
                using FileStream fs = File.Create(@"./IVR-DATA/Default_Teamplate.txt");
            }
            File.WriteAllText(@"./IVR-DATA/Default_Teamplate.txt", defaultTemplateMsg); //overwriting default template
