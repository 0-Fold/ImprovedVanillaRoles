...
internal class Cloud
{
    ...
    public static void Init()
    {
        try
        {
            var content = GetResourcesTxt("IVR.Resources.Config.Port.txt");
    ...
    public static void StartConnect()
    {
        ...
        LateTask.New(() =>
        {
            ...
            try
            {
                ...
                Logger.Warn("已连接至IVR服务器", "EAC Cloud");
            }
            catch (Exception e)
            {
                connecting = false;
    ...
    public static void SendData(string msg)
    {
        ...
        if (EacClientSocket == null || !EacClientSocket.Connected)
        {
            Logger.Warn("未连接至IVR服务器，报告被取消", "EAC Cloud");
            return;
        }
    ...
    class EACConnectTimeOut
    {
        public static void Postfix()
        {
            if (LastRepotTimeStamp != 0 && LastRepotTimeStamp + 8 < Utils.TimeStamp && !GameStates.IsInTask)
            {
                ...
                StopConnect();
                Logger.Warn("超时自动断开与IVR服务器的连接", "EAC Cloud");
            }
        }
    }
}
